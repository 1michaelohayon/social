...
const SingleMessage = ({ message }: { message: Message }) => {
    ...
    return <Container style={message.reply ? { backgroundColor: "#FFFAF0" } : {}}>
        <TopContainer>
        ...
        <BottomContainer>
            <div>
                ♡ {updatedLikes} {!user
                    ...
                    : liked || alreadyLiked()
                        ? ""
                        : <SecondryButton onClick={handleLike}>אהבתי</SecondryButton>
                }

            </div>

            <Togglable children={<ReplyForm replyTo={Number(message.id)} />} buttonLabel="תגובה" />
        </BottomContainer>
    </Container>
}
